...
class XORModel(nn.Module):
    def __init__(self):
        ...
        self.layer2 = nn.Linear(4, 2)  # 두 번째 선형 레이어, 입력 크기 4, 출력 크기 2
        self.layer3 = nn.Linear(2, 1)  # 세 번째 선형 레이어, 입력 크기 2, 출력 크기 1
        self.relu = nn.ReLU()  # ReLU 활성화 함수
        self.sigmoid = nn.Sigmoid()  # Sigmoid 활성화 함수

    def forward(self, x):
        x = self.relu(self.layer1(x))  # 첫 번째 레이어와 ReLU 적용
        x = self.relu(self.layer2(x))  # # 두 번째 레이어와 ReLU 적용
        x = self.sigmoid(self.layer3(x))  # 세 번째 레이어와 Sigmoid 적용
        return x
